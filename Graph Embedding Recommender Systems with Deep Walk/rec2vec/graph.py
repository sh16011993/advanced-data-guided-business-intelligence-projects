...
def records_to_graph():
    ...
    nodedict = {}
    # YOUR CODE HERE
    # Initializing Id to 0
    id = 0
    # Creating nodes from left to right in the diagram starting with user
    ...
    ## Creating movie and movie-rating nodes
    for movie in movies:
        node = Node(id, movie, 'movie')
        nodelist.append(node)
        nodedict[movie] = node
        id = id + 1
        for mr in range(1, 6):
            node = Node(id, str(movie) + '_' + str(mr), 'movie-rating')
            nodelist.append(node)
            nodedict[str(movie) + '_' + str(mr)] = node
            id = id + 1
	
	## Creating director nodes
    ...
    # YOUR CODE HERE
	# Adding all the edges from left to right
    ## Creating edges b/w user and movie-ratings
    for user in ratings:
        userNode = nodedict[user]
        userMoviesDict = ratings[user]
        for userMovie in userMoviesDict:
            userMovieRating = userMoviesDict[userMovie]
            userMovieRatingNode = nodedict[str(userMovie) + '_' + str(userMovieRating)]
            userNode.neighbors.append(userMovieRatingNode)
            userMovieRatingNode.neighbors.append(userNode)
		
    ## Creating edges b/w movie-ratings and movies
    for movie in movies:
        movieNode = nodedict[movie]
        for movieRating in range(1, 6):
            movieRatingNode = nodedict[str(movie) + '_' + str(movieRating)]
            movieNode.neighbors.append(movieRatingNode)
            movieRatingNode.neighbors.append(movieNode)

    ## Creating edges b/w movies and directors, b/w movies and actors, b/w movies and genres
    for movie in movies:
        movieNode = nodedict[movie]
		
        ## Edges b/w movies and directors
        if(movies[movie].director != None):
            directorNode = nodedict[director]
            directorNode.neighbors.append(movieNode)
            movieNode.neighbors.append(directorNode)
			
        ## Edges b/w movies and actors
